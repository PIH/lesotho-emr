...
the config-pihemr project, then build the config-lesotho project, (pulling in any changes to config-pihemr),
...
*both* the config-pihemr and config-lesotho projects for changes and redeploy when there are changes.  It runs
...
`mvn clean compile -DserverId=lesotho` - Will compile as above, and copy the resulting configuration to `~/openmrs/lesotho/configuration`
